...
export default async function PlatformSearchPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const products = [
    {
      icon: Search,
      title: t("home.platform.fullTextSearch"),
      description: t("home.platform.searchDesc"),
    },
    {
      icon: FileSearch,
      title: t("home.platform.documentSearch"),
      description: t("home.platform.searchDesc"),
    },
    {
      icon: Database,
      title: t("home.platform.indexing"),
      description: t("home.platform.searchDesc"),
    },
    {
      icon: Filter,
      title: t("home.platform.facetedSearch"),
      description: t("home.platform.searchDesc"),
    },
  ];
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight text-foreground leading-tight text-balance">
                {t("home.platform.searchTitle")}
              </h1>
              <p className="mt-6 text-lg sm:text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
                {t("home.platform.searchDesc")}
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {products.map((product) => (
                <div key={product.title} className="p-6 rounded-lg border border-border bg-card hover:border-foreground/20 transition-colors group">
                  <div className="w-16 h-16 bg-cyan-500/20 rounded-xl flex items-center justify-center mb-6 group-hover:opacity-80 transition-opacity">
                  ...
                  <span className={`text-sm font-semibold flex items-center gap-1 text-cyan-400 hover:opacity-80 transition-opacity`}>
