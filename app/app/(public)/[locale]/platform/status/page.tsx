...
export default async function PlatformStatusPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const products = [
    {
      icon: Activity,
      title: t("home.platform.realTimeMonitoring"),
      description: t("home.platform.statusDesc"),
    },
    {
      icon: CheckCircle2,
      title: t("home.platform.statusPage"),
      description: t("home.platform.statusDesc"),
    },
    {
      icon: AlertTriangle,
      title: t("home.platform.incidentAlerts"),
      description: t("home.platform.statusDesc"),
    },
    {
      icon: Server,
      title: t("home.platform.uptimeTracking"),
      description: t("home.platform.statusDesc"),
    },
  ];
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight text-foreground leading-tight text-balance">
                {t("home.platform.statusTitle")}
              </h1>
              <p className="mt-6 text-lg sm:text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
                {t("home.platform.statusDesc")}
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {products.map((product) => (
                <div key={product.title} className="p-6 rounded-lg border border-border bg-card hover:border-foreground/20 transition-colors group">
                  <div className="w-16 h-16 bg-red-500/20 rounded-xl flex items-center justify-center mb-6 group-hover:opacity-80 transition-opacity">
                  ...
                  <span className={`text-sm font-semibold flex items-center gap-1 text-red-400 hover:opacity-80 transition-opacity`}>
